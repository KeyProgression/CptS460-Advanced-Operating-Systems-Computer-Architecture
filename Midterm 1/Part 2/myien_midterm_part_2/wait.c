...
{
  PROC *everyProc;
  PROC *tempSleepPROCList = 0;
  int SR = int_off();         // Disable IRQ Interrupts and return CPSR
                                // Current Program Status Register (CPSR)
  while (everyProc = dequeue(&sleepList))
  {
    if (everyProc->event == event)
    {
      kprintf("\nPROC[%d] found, matching EVENT: %d\n\t |-> WOKE UP PROC[%d]\n", everyProc->pid, event, everyProc->pid);
      everyProc->status = READY;       // READY == 1
      enqueue(&readyQueue, everyProc);
      }
      else
      {
        enqueue(&tempSleepPROCList, everyProc);
      }
    }
    sleepList = tempSleepPROCList;
...
int timerWakeup(int event)
{
    ...
    while (everyProc = dequeue(&sleepList))
    {
        if (everyProc->event == event)
        {
            ...
            everyProc->status = READY;
            enqueue(&readyQueue, everyProc);
        }
        else
        {
            inOrderTimerRequeue(&tempSleepPROCList, everyProc);
        }
    }
    sleepList = tempSleepPROCList;   
